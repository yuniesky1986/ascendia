...
class L10nEsAeatMod347Report(models.Model):
    ...
    def _invoice_amount_get(self, invoices, refunds):
        ...
        invoice_amount = sum(x.amount_total for x in invoices)
        refund_amount = sum(x.amount_total for x in refunds)
    ...
    def _cash_moves_group(self, moves):
        ...
        for move_line in moves:
            ...
            if invoices:
                invoice = invoices[0]
                fy_id = invoice.date_invoice.strftime('%Y')
...
class L10nEsAeatMod347PartnerRecord(models.Model):
    ...
    def _get_quarter_invoice_totals(self):
        ...
        def _invoices_sum(invoices, refunds, quarter):
            mach = {'first': '1', 'second': '2', 'third': '3', 'fourth': '4'}
            quarter_num = mach[quarter]
            starting_month = 1 + (int(quarter_num) - 1) * 3
            ending_month = starting_month + 2
            date_start = fields.Date.from_string(
                '%s-%s-01' % (self.report_id.year, starting_month))
            date_end = fields.Date.from_string(
                '%s-%s-%s' % (
                    self.report_id.year, ending_month,
                    monthrange(self.report_id.year, ending_month)[1]))

            return (
                sum(x.invoice_id.amount_total for x in invoices
                    if datetime.strptime(x.invoice_id.date_invoice, "%Y-%m-%d").date() >= date_start
                    and datetime.strptime(x.invoice_id.date_invoice, "%Y-%m-%d").date() <= date_end) -
                sum(x.invoice_id.amount_total for x in refunds
                    if datetime.strptime(x.invoice_id.date_invoice, "%Y-%m-%d").date() >= date_start
                    and  datetime.strptime(x.invoice_id.date_invoice, "%Y-%m-%d").date() <= date_end))
    ...
    def _get_quarter_cash_totals(self):
        def _cash_moves_sum(records, quarter):
            ...
            return (sum(x.amount for x in records
                        if datetime.strptime(x.move_line_id.date, "%Y-%m-%d").date() >= date_start and  
                        datetime.strptime(x.move_line_id.date, "%Y-%m-%d").date() <= date_end))
